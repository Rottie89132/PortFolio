    ...
    <div class="fixed  w-full h-full p-4 pb-5 select-none md:pb-0 md:p-0 ">
            <div :class="!Installed ? 'h-full' : 'h-[95%]'" class="flex items-center justify-center w-full md:h-full p-5 md:rounded-none rounded-3xl bg-gradient-to-r from-teal-50 to-yellow-50 dark:from-[#131313] dark:to-[#22221E] overflow-auto">
                <div class=" md:w-[60%]">
                    ...
                    <p v-else class="mt-4 text-base font-semibold text-red-600 md:text-lg md:w-[65%] ">{{ message.message }}</p>
                    <div class="flex items-center gap-4 mt-6">
                        <button :disabled="loading" @click="handleInstall" v-if="message.statusCode" class="flex items-center gap-2 px-5 py-2 text-sm font-semibold dark:text-black dark:bg-white dark:ring-white text-white bg-black ring-2 ring-black rounded-xl">
                            <p>Opnieuw proberen</p>
                        </button>
                        <button v-else :disabled="loading" @click="navigateTo('/dashboard/Repos')" class="flex items-center gap-2 px-5 py-2 text-sm font-semibold dark:text-black dark:bg-white dark:ring-white text-white bg-black ring-2 ring-black rounded-xl">
                            <p>Handmatig doorshakelen</p>
                        </button>
                    </div>
                </div> 
            </div>
...
const handleInstall = () => {
    ...
    setTimeout(async () => {
        loading.value = false
        const { error } = await useFetch(`/api/repo/auth/${code}/${installation_id}`, { method: 'POST' })
