			...
			<div class="w-full h-fit mt-6 md:my-10 xl:mt-20 md:w-[89.2%]">
				<div class="flex items-center justify-between gap-3 mb-3">
					...
					<div v-if="items.length > 0 && hidebuttons.total > 1" class="flex items-center gap-2">
						<button aria-label="Vorige" v-if="hidebuttons.page > 1" :disabled="loading" class="p-2 bg-[#F7F7F7] dark:bg-[#111111] dark:text-white flex items-center justify-center rounded-xl" @click="PreviousPage">
							<Icon name="material-symbols:keyboard-arrow-left" size="1.2em"></Icon>
						</button>
						<button aria-label="Volgende" v-else :disabled="loading" class="p-2 bg-[#F7F7F7] dark:bg-[#111111] dark:text-white flex items-center justify-center rounded-xl" @click="NextPage">
							<Icon name="material-symbols:keyboard-arrow-right" size="1.2em"></Icon>
						</button>
						<button aria-label="Volgende" v-if="hidebuttons.page > 1" :disabled="loading" class="p-2 bg-[#F7F7F7] dark:bg-[#111111] dark:text-white flex items-center justify-center rounded-xl" @click="NextPage">
						...
						<button aria-label="Vorige" v-else :disabled="loading" class="p-2 bg-[#F7F7F7] dark:bg-[#111111] dark:text-white flex items-center justify-center rounded-xl" @click="PreviousPage">
				...
				<div class="flex items-center gap-2 dark:text-white" v-if="loadingIndicater"><icon name="mingcute:loading-fill" class="animate-spin" size="1.4em"></icon>Repositories worden ingeladen</div>
				...
				<div v-else class="p-3 bg-[#F7F7F7] dark:bg-[#111111] h-fit rounded-2xl transition-transform">
					<div class="w-full h-full overflow-y-auto snap-y snap-proximity rounded-xl scroll-smooth">
						<div v-for="(item, index) in items" :key="item.repo_id" class="mb-3 delay-100 last:mb-0 animate-fade-in">
							<div v-if="item.loaded" class="p-3 bg-white dark:bg-neutral-900 md:snap-end snap-center h-fit rounded-xl">
								...
								<div class="flex items-center gap-3 justify-between font-semibold select-none">
									<div class="flex items-center gap-3">
										<a v-if="item.homepage" :href="item.homepage" target="_blank" class="flex items-center justify-center px-3 py-1 mt-3 mb-1 text-xs dark:text-black dark:bg-white font-semibold dark:ring-white text-white bg-black rounded ring-2 ring-black">Bezoeken</a>
