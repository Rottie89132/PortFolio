							...
							<div v-for="(item, index) in Items" :key="index"
								...
								<div v-if="item.loaded"
									...
									<div class="flex items-center justify-between gap-2">
										<h2 class="text-xs dark:text-white font-bold truncate md:text-base">Je hebt een
											bericht verstuurd</h2>
										<p class="text-[0.65em] dark:text-white md:text-sm opacity-70">
											{{ useTimeAgo(item.created_at).value }}
										</p>
									</div>
									<p
										class="dark:text-white text-balance mt-1 md:mt-0 mr-11 xl:mr-32 text-xs line-clamp-2 opacity-80">
										{{ item.message }}</p>
