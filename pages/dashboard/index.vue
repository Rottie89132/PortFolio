            ...
            <div class=" w-full h-fit mt-6 md:my-10 xl:mt-20 md:w-[89.2%]  ">
                ...
                <div class="flex items-center justify-between gap-8 mb-6 md:justify-normal ">
                    <div>
                        ...
                        <p class="leading-[1.1em] dark:text-white text-sm">Er zijn <span class="font-black ">{{
                            MonthlyVisted }}</span> bezoekers geweest deze maand</p>
                    </div>
                    <div>
                        <p class="font-black dark:text-white ">Berichten</p>
                        <p class="leading-[1.1em] dark:text-white text-sm">Er zijn <span class="font-black ">{{
                            MonthlyMessages }}</span> berichten verzonden deze maand</p>
                    </div>
                </div>
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <ClientOnly>
                        <NuxtLink :to="`/dashboard/Repos?Page=${currentPage}`"
                            class="text-indigo-600 dark:text-indigo-500 text-[0.65em] text-right ">Bekijken</NuxtLink>
                        <template #fallback>
                ...
                <div v-else
                    :class="Repos.length <= 2 ? 'h-fit' : Messages.length > 2 ? 'h-[15.6svh] md:h-[17.3svh] xl:h-[15.5svh]' : 'h-[22.4svh] md:h-[25svh] xl:h-[22.5svh]'"
                    class=" p-3 bg-[#F7F7F7] dark:bg-[#111111] rounded-2xl mb-6 ">
                    <div class="w-full h-full overflow-y-auto md:pr-2 rounded-xl snap-y snap-proximity scroll-smooth ">
                        <div v-for="(item, index) in Repos" :key="index" :id="`Repo:${index}`"
                            class="w-full p-3 mb-3 bg-white dark:bg-neutral-900 last:mb-0 snap-start rounded-xl h-fit">
                            <div class="flex items-center justify-between gap-2 ">
                                ...
                                <div class=" flex items-center gap-2 ">
                                    <a :href="item.html_url" target="_blank"
                                        class="text-xs font-medium dark:text-indigo-500 text-indigo-600 ">{{ item.name
                                        }}</a>
                                    <icon name="ri:external-link-line" class=" text-indigo-600 dark:text-indigo-500"
                                        size="1em"></icon>
                                </div>
                            </div>
                        </div>
