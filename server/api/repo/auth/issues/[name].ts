...
export default defineEventHandler((event) => {
    return new Promise(async (resolve, reject) => {
        ...
        const { name: repositoryName } = getRouterParams(event);
        ...
        return await useGetIssues(token, currentPage, repositoryName, resolve).catch(async (error: any) => {
            const OctoKitData = await useOctokit(GitConnection.installationId)
            await useSaveInstall("Update", user, OctoKitData)
            return await useGetIssues(OctoKitData.token, currentPage, repositoryName, resolve).catch((error: any) => {
                return reject({
                    statusCode: 404,
                    statusMessage: "Not Found",
