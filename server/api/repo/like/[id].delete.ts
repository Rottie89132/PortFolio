export default defineEventHandler((event) => {
    return new Promise(async (resolve, reject) => {
        ...
        const RemoveLike = await Repositories.findOneAndUpdate({ repo_id: id }, {
            $pull: { liked_byId: user.Id }
        })

        if (!RemoveLike) return reject({
            statusCode: 500,
        ...
        return resolve({
            statusCode: 200,
            statusMessage: "OK",
            message: "You have successfully unliked this repository."
        })
    })
})