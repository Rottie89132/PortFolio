                                ...
                                <Form class="" @submit="handleRequest" :validation-schema="schema" v-slot="{ meta }">
                                    ...
                                    <div v-if="type == 'Inloggen'" class="flex justify-between items-center">
                                        <div @click="forgotPassword"
                                            class="text-sm select-none cursor-pointer text-neutral-800 hover:text-neutral-900">
                                            Wachtwoord vergeten?
                                        </div>
                                    </div>
                                    ...
                                    <span role="alert" v-if="MessageSend && type == 'Contact'"
                                        class="flex text-xs text-left ">Je bericht is vestuured</span>
                                    <span role="alert" v-if="MessageSend && type == 'Aanmelden'"
                                        class="flex text-xs text-left ">Je account is aangemaakt, controleer je email om je
                                        account te activeren</span>
                                    <span role="alert" v-if="MessageSend && type == 'Vergeten'"
                                        class="flex text-xs text-left "> Er is een email verstuurd naar het opgegeven email
                                        adres
                                    </span>
                                </Form>
                            </div>
...
const handleRequest = async (values: any, actions: any) => {
    ...
    const { data, error, pending, refresh }: Record<string, any> = AuthModule.value ?
        ...
        await useCsrfFetch('/api/berichten', { method: "post", body: values }) : type.value == "Vergeten" ? 
        await useCsrfFetch('/api/forgot', { method: "post", body: values }) : 
