                    ...
                    <Transition name="modalDelay">
                        <div ref="modalDelay" v-if="DelayStatus">
                            <div class="p-8 bg-white md:max-w-[30vw] rounded-2xl ">
                                ...
                                <div>
                                    <p class=" mb-6 font-medium text-sm opacity-70"> Werk je huidige curriculum vitae bij
                                        en upload vervolgens je nieuwe curriculum vitae.</p>
                                    <Form @submit="handleRequest" :validation-schema="schema" v-slot="{ meta }">
                                        <FieldUpload :name="'CV'" v-model="file" :type="'file'"
                                            :label="'Curriculum Vitae'" />
                                        <div class=" text-xs mb-4 mt-3 text-green-700" v-if="meta.valid">{{ fileName }}
                                        </div>
                                        ...
                                        <span role="alert" v-if="MessageSend" class="flex text-xs text-left ">Je Curriculum
                                            vitae is geüpload</span>
                                    </Form>
                                </div>
                            </div>
                        </div>
...
const { status, DelayStatus, title } = defineModels<{
    status: Boolean, DelayStatus: Boolean, title: String
}>()
