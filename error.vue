<template>
    <div class="fixed w-full h-full p-4 pb-5 select-none md:pb-0 md:p-0 ">
        <div :class="!Installed ? 'h-full' : 'h-[95%]'" class="flex items-center justify-center w-full md:h-full p-5 md:rounded-none rounded-3xl bg-gradient-to-r from-teal-50 to-yellow-50 dark:from-[#131313] dark:to-[#22221E]">
            <div class="">
                <h1 class="text-[8em] md:text-[13em] dark:text-white font-black leading-[1em] mb-2">{{ Error?.statusCode }}</h1>
                <p v-if="Error?.statusCode == 404" class="font-bold  dark:text-white md:text-[2em] text-[2em] opacity-60 leading-[1em]">De aangevraagde pagina is niet gevonden</p>
                <p v-else class="font-bold md:text-[2em]  dark:text-white text-[1.8em] opacity-60 leading-[1em]">{{ Error?.statusMessage }}</p>
                <p class="mt-2 dark:text-white">{{ Error?.message }}</p>
